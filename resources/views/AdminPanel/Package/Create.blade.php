...
const foodsContainer = document.querySelector('.foods-container');
    ...
    document.querySelectorAll('.food-total-hidden').forEach(ft => {
...
destinationSelect.addEventListener('change', function() {
    ...
    // Fetch hotels
    fetch(`/destinations/${destinationId}/hotels`)
        .then(res => res.json())
        .then(data => {
    ...
    fetch(`/destinations/${destinationId}/foods`)
        ...
        .then(data => {
            ...
            data.forEach(food => {
                ...
                col.innerHTML = `
                    <label class="form-label fw-bold">${food.name} ($${food.price})</label>
                    <input type="number" 
                           name="foods[${food.id}][qty]" 
                           class="form-control rounded-3 shadow-sm food-total" 
                           value="0" 
                           data-price="${food.price}" />
                    <input type="hidden" 
                           name="foods[${food.id}][total]" 
                           class="food-total-hidden" 
                           value="0" />
                `;

                foodsContainer.appendChild(col);

                const qtyInput = col.querySelector('input.food-total');
                const totalInput = col.querySelector('input.food-total-hidden');

                // Update total for this food on input
                qtyInput.addEventListener('input', function() {
                    const qty = parseFloat(this.value) || 0;
                    totalInput.value = (qty * parseFloat(this.dataset.price)).toFixed(2);
                    updateGrandTotal();
                });
            });
        ...
        .then(data => {
            ...
            data.forEach(food => {
                ...
                col.innerHTML = `
                    ...
                    <input type="number" class="form-control rounded-3 shadow-sm food-total" value="0" data-price="${food.price}" />
                `;
                foodsContainer.appendChild(col);
            });

            // Add listener to food inputs to update grand total
